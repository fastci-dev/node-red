...
/**
 * Run system core with event-logger integration
...
function logLines(id,type,data) {
    events.emit("event-logger", {id:id,payload:{ts: Date.now(),data:data,type:type}});
}

module.exports = {
    /**
     * Run a system command with stdout/err being emitted as 'event-logger' events
     ...
     * @param {Boolean} emit - whether to emit events to the event-logger for each line of stdout/err
    ...
    run: function(command,args,options,emit) {
        ...
        emit && events.emit("event-logger", {ts: Date.now(),id:invocationId,payload:{ts: Date.now(),data:command+" "+args.join(" ")}});
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                ...
                emit && events.emit("event-logger", {id:invocationId,payload:{ts: Date.now(),data:"rc="+code}});
