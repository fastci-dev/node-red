   ...
   if (!this.connected) {
       ...
       self.client = mqtt.createConnection(this.port,this.host,function(err,client) {
             if (err) {
                ...
                //util.logger('[mqtt] ['+self.uid+'] connection error 1 : '+inspect(err));
                self.emit('connectionlost',err);
                return;
             }
             client.on('close',function(e) {
                   //util.logger('[mqtt] ['+self.uid+'] on close');
                   clearInterval(self.watchdog);
             ...
             client.on('error',function(e) {
                   //util.logger('[mqtt] ['+self.uid+'] on error : '+inspect(e));
                   clearInterval(self.watchdog);
                   if (self.connected) {
                      self.connected = false;
                      self.emit('connectionlost',e);
             ...
             client.on('connack',function(packet) {
                   if (packet.returnCode === 0) {
                      self.watchdog = setInterval(function(self) {
                            ...
                            //util.logger('[mqtt] ['+self.uid+'] watchdog '+inspect({connected:self.connected,connectionError:self.connectionError,pingOutstanding:self.pingOutstanding,now:now,lastOutbound:self.lastOutbound,lastInbound:self.lastInbound}));
                            ...
                            if (now - self.lastOutbound > self.options.keepalive*500 || now - self.lastInbound > self.options.keepalive*500) {
                               if (self.pingOutstanding) {
                                  //util.logger('[mqtt] ['+self.uid+'] watchdog pingOustanding - disconnect');
                                  try {
                                     self.client.disconnect();
                                  } catch (err) {
                                  }
                               } else {
                                  //util.logger('[mqtt] ['+self.uid+'] watchdog pinging');
             ...
             client.on('pingresp',function(packet) {
                   //util.logger('[mqtt] ['+self.uid+'] received pingresp');
                   self.lastInbound = (new Date()).getTime()
                   self.pingOutstanding = false;
             });
