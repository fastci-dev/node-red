RED.diff = (function() {
    ...
    function createNodeDiffRow(node,stats,CurrentDiff) {
        ...
        // console.logger(node.id,localDiff.added[node.id],remoteDiff.added[node.id],localDiff.deleted[node.id],remoteDiff.deleted[node.id],localDiff.changed[node.id],remoteDiff.changed[node.id])
        var def = RED.nodes.getType(node.type);
        if (def === undefined) {
    ...
    function resolveDiffs(localDiff,remoteDiff) {
        ...
        // console.logger(diff.resolutions);
        // console.logger(conflicted);
        return diff;
    }
    ...
    function showDiff(diff,options) {
        ...
        var trayOptions = {
            ...
            open: function(tray) {
                ...
                refreshConflictHeader(diff);
                // console.logger("--------------");
                // console.logger(localDiff);
                // console.logger(remoteDiff);
    ...
    function createExpandLine(start,end,diffLines) {
        ...
        diffRow.on("click", function(evt) {
            // console.logger(start,end,diffLines.length);
            if (end - start > 20) {
                var startPos = $(this).offset();
                // console.logger(startPos);
                if (start > 0) {
