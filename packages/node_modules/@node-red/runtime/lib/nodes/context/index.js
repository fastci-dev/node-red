...
function load() {
    return new Promise(function(resolve,reject) {
        ...
        if (plugins && Object.keys(plugins).length > 0) {
                ...
                if (plugins.hasOwnProperty(pluginName)) {
                    ...
                    if (plugins[pluginName].hasOwnProperty("module")) {
                        ...
                        try {
                            ...
                            log.info(log._("context.logger-store-init", {name:pluginName, info:"module="+moduleInfo}));
                        } catch(err) {
            ...
            log.info(log._("context.logger-store-init", {name:"default", info:"module=memory"}));
...
function getContextStorage(storage) {
    if (stores.hasOwnProperty(storage)) {
        ...
        if (storage !== defaultStore) {
            // It isn't the default store either, so logger it
